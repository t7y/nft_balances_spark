...
def write_schemas(output_dir='output/schemas'):
    ...
    LOG_SCHEMA_JSON = {
      ...
      "required": ["address", "topics", "data", "blockNumber", "transactionHash", "transactionIndex", "blockHash", "logIndex"]
    }
    ...
    TRANSACTION_SCHEMA_JSON = {
      ...
      "properties": {
        ...
        "accessList": {"type": "array", "description": "Access list for the transaction", "items": {"type": "object"}},
...
def define_spark_schema():
    """Define and return the Spark schema for parsing the JSON column."""
...
def read_and_parse_csv(spark, csv_path, block_schema):
    """Read the CSV file and parse the JSON column into a DataFrame column 'block'."""
    df = spark.read.option("header", True).csv(csv_path)
    return df.withColumn("block", from_json(col("json"), block_schema))

def explode_logs(df):
    """Explode the transactions and logs so each row represents a single log event."""
    df_tx = df.select("block.*").withColumn("tx", explode(col("transactions")))
    return df_tx.withColumn("log", explode(col("tx.logs")))

def process_erc721(df_logs):
    """Process ERC-721 transfer events and return incoming/outgoing DataFrames."""
...
def process_erc1155_single(df_logs):
    """Process ERC-1155 TransferSingle events and return incoming/outgoing DataFrames."""
...
def process_erc1155_batch(df_logs):
    """Process ERC-1155 TransferBatch events and return incoming/outgoing DataFrames."""
    ERC1155_TRANSFER_BATCH_SIG = "0x4a39dc06d4c0dbc64b70b5e7f3c8b"  # adjust full value as needed
...
def main():
    ...
    csv_path = "./input/bwt_single.csv"
    output_path = "./output/"

    spark = create_spark_session()
    write_schemas()  # Optional: write JSON schema files

    # Define the Spark schema and read/parse the CSV file.
    block_schema = define_spark_schema()
    df_parsed = read_and_parse_csv(spark, csv_path, block_schema)
    ...
    # Compute net balance per wallet and token_id.
    nft_balances = df_transfers.groupBy("wallet", "token_id").agg(
        spark_sum("delta").alias("balance")
    )
    nft_balances.show(truncate=False)
    nft_balances.write.mode("overwrite").parquet(output_path)
